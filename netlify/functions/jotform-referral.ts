...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    ...
    const db = getDb();
    const result = await processSubmission(db, "REFERRAL", body);

    logger.info({ result: { action: result.action } }, "Referral processed");

    return jsonResponse(result.success ? 200 : 422, result);
  } catch (err) {
    logger.error({ err }, "Referral handler error");
...
function parseBody(event: HandlerEvent): Record<string, unknown> | null {
  ...
  try {
    ...
    // JotForm may send form-encoded data
    try {
