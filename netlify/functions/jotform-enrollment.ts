...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    ...
    const db = getDb();
    const result = await processSubmission(db, "ENROLLMENT", body);

    logger.info({ result: { action: result.action } }, "Enrollment processed");

    return jsonResponse(result.success ? 200 : 422, result);
  } catch (err) {
    logger.error({ err }, "Enrollment handler error");
